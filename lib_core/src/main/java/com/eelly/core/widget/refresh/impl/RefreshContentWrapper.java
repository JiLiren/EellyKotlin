...
public class RefreshContentWrapper implements IRefreshContent {
    ...
    protected void findScrollableView(View content, IRefreshKernel kernel) {
    ...
    protected void wrapperCoordinatorLayout(ViewGroup layout, final IRefreshLayout refreshLayout) {
    ...
    @Override
    public void setUpComponent(IRefreshKernel kernel, View fixedHeader, View fixedFooter) {
    ...
    public AnimatorUpdateListener scrollContentWhenFinished(final int spinner) {
        if (mScrollableView != null && spinner != 0) {
            if ((spinner < 0 && ScrollBoundaryUtil.canScrollDown(mScrollableView)) || (spinner > 0 &&
                    ScrollBoundaryUtil.canScrollUp(mScrollableView))) {
